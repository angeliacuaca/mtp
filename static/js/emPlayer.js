    ...
    SC.resolve(url).then(function (track) {
        return SC.stream('/tracks/' + track.id).then(function (player) {
            ...
            if ((!plist.hasChildNodes()) || (!exist)) {
                ...
                dropBtn.setAttribute('style', "border: none; height: 18px; width: 18px; background-size: 100%; background-color: transparent; background-image: url('/static/images/DropTrackIcon.png'); background-repeat: no-repeat;background-position: center;");
...
// this function is used if you only need load a track without creating a playlist and cover pic
function onlyPlayTrack(url) {
    SC.resolve(url).then(function (track) {
        return SC.stream('/tracks/' + track.id).then(function (player) {
            ...
            trackInfo.innerHTML = track.title + " - " + track.user.username;
            document.getElementById('duration').innerHTML = updateTime(duration);
            if (track.artwork_url != null && document.getElementById('trackCover') != null)
                document.getElementById('trackCover').src = track.artwork_url.replace(/large/i, 'badge');
            else
                document.getElementById('trackCover').src = track.user['avatar_url'];
            if (currentPlayer)
